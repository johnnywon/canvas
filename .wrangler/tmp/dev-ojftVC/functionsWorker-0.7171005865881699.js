...
// .wrangler/tmp/pages-ES4nkd/functionsWorker-0.7171005865881699.mjs
...
async function verifyAccess(db, canvasId, email) {
...
app.post("/api/canvases", async (c) => {
  ...
  return c.json({ id, name, owner_email: email, created_at: (/* @__PURE__ */ new Date()).toISOString(), updated_at: (/* @__PURE__ */ new Date()).toISOString() }, 201);
});
app.get("/api/canvases/:id", async (c) => {
  ...
  const [nodesRes, edgesRes] = await Promise.all([
    c.env.DB.prepare("SELECT * FROM nodes WHERE canvas_id = ?").bind(canvasId).all(),
    c.env.DB.prepare("SELECT * FROM edges WHERE canvas_id = ?").bind(canvasId).all()
  ]);
  return c.json({
    canvas,
...
app.patch("/api/canvases/:id/state", async (c) => {
  ...
  const { nodes, edges } = await c.req.json();
...
app.get("/api/canvases/:id/comments", async (c) => {
  ...
  ).bind(canvasId, parentType, parentId).all();
  return c.json(results);
});
...
var onRequest = /* @__PURE__ */ __name2((ctx) => app.fetch(ctx.request, ctx.env, ctx), "onRequest");
var routes = [
  {
    routePath: "/api/:path*",
    mountPath: "/api",
...
// .wrangler/tmp/bundle-Nqtaa8/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Nqtaa8/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7171005865881699.js.map
